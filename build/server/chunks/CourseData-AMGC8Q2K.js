...
const CourseData = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  return `   <div class="course-container svelte-1dd25co"><div style="width: 80vw;position:relative; left:-50px; ">${``}</div>  <div class="courseSection svelte-1dd25co"><div class="course svelte-1dd25co"><div class="courseImg svelte-1dd25co"><img${add_attribute("src", src, 0)} alt="" style="object-fit: cover;" class="svelte-1dd25co"></div> <button class="${escape(null_to_empty("joinButton"), true) + " svelte-1dd25co"}" ${""}>${escape("Join")}</button> <div class="courseTxt svelte-1dd25co"><h2>${escape(title)}</h2> <!-- HTML_TAG_START -->${description}<!-- HTML_TAG_END --></div></div></div> ${``}  ${``} ${``} </div>`;
});

export { CourseData as C };
//# sourceMappingURL=CourseData-AMGC8Q2K.js.map
