...
const Button = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  return `<button class="button svelte-ou0p7a"${add_attribute("style", "border-radius:" + radius + ";" + (padding ? "padding:0px;" : "padding:7px 15px;"), 0)}><div style="display: flex;align-items:center;"><div>${escape(text ? text : "")}</div> ${src ? `<img${add_attribute("src", src, 0)}${add_attribute("style", size ? `width:${size.w}px;height:${size.w}px;` : "", 0)}>` : ``}</div></button>`;
});

export { Button as B };
//# sourceMappingURL=Button-D2DSi2ku.js.map
